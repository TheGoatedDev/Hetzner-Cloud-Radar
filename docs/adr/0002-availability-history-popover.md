...
- Each cell's runs are cached client-side for the page session and server-side for one poll cycle (5 minutes), so concurrent hovers collapse to at most one database read per cell per cycle.
- A gap of more than 900 seconds (3× the 5-minute poll cadence) between consecutive observations renders as an `unknown` segment; shorter gaps are treated as continuation of the prior state. This makes poller outages visible rather than masquerading as availability.
