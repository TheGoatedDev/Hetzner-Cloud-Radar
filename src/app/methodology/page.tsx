            ...
            <section className="flex flex-col gap-5 pt-10">
                ...
                <CopyBlock title="What we poll" first>
                    <p className={PROSE}>
                        ...
                        from the public Hetzner Cloud API. The endpoint returns
                        each datacentre with three lists per server type:{" "}
                        <span className="font-mono text-ink">supported</span>,{" "}
                        <span className="font-mono text-ink">available</span>,
                        and{" "}
                ...
                <CopyBlock title="The four cell states">
                    <p className={PROSE}>
                        Each cell on the matrix collapses the API response into
                        one of four states. The Hetzner API itself only exposes
                        binary purchase availability, so &ldquo;limited&rdquo;
                        is derived from poll history.
                    </p>
                ...
                <CopyBlock title="Cadence and freshness">
                    <p className={PROSE}>
                        Polls run every {POLL_CADENCE} from a single observation
                        point. A poll is considered successful only when the API
                        responds with a full datacentre list and no rate-limit
                        errors. Failed polls are recorded but never used to
                        decide cell state, so a transient outage on the radar
                        side never reads as a stock-out on yours.
                    </p>
                    <p className={PROSE}>
                        The masthead timestamp on every page is the time of the
                        most recent successful poll. If you ever see it stale by
                        more than a few minutes, the radar is the one having a
                        bad day.
                    </p>
                </CopyBlock>
                ...
                <CopyBlock title="What counts as a dispatch">
                    <p className={PROSE}>
                        A dispatch is filed when a cell transitions in a way
                        that matters:
                    </p>
                    <ul className="flex flex-col gap-1.5 pl-5 max-w-[68ch] font-sans text-base leading-[1.65] text-ink-soft list-disc marker:text-ink-faint">
                        <li>
                            <span className="text-ink">Sold out:</span> a
                            previously available cell becomes unavailable, and
                            is still unavailable at publication time.
                        </li>
                        <li>
                            <span className="text-ink">Restocked:</span> a
                            previously sold-out cell becomes available again.
                        </li>
                        <li>
                            <span className="text-ink">Rollout:</span> a
                            previously not-offered cell becomes available,
                            indicating new regional capacity.
                        </li>
                    </ul>
                    <p className={PROSE}>
                        Brief flickers within a single day raise a cell to{" "}
                        <span className="font-mono text-ink">limited</span> on
                        the matrix but do not generate a separate dispatch.
                    </p>
                </CopyBlock>

                <CopyBlock title="Independence">
                    <p className={PROSE}>
                        Hetzner Cloud Radar is not affiliated with Hetzner
                        Online GmbH. The radar reads only the public API. It
                        does not represent Hetzner, does not speak on their
                        behalf, and does not coordinate on incident
                        communication. When the radar disagrees with an official
                        statement, the radar reports what it observed and lets
                        the reader decide.
                    </p>
                </CopyBlock>
            </section>
