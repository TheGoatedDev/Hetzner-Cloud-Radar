...
function tooltipAnchor(index: number, total: number) {
    ...
    if (ratio > 0.9)
        return { transform: "translate(-100%, -100%)", offsetX: 4 };
...
export function SupplyChart({ days }: { days: SupplyDay[] }) {
    ...
    const statLines = [
        ...
        (line, index, lines) =>
            index ===
            lines.findIndex((candidate) => candidate.value === line.value),
    );
    ...
    return (
        <figure className="flex flex-col gap-2">
            <table className="sr-only">
                <caption>
                    Available, limited, and sold-out cells per day, last{" "}
                    {days.length} days. Maximum {maxStack} cells. Available
                    average {formatSupplyStat(stats.average)}, min {stats.min},
                    max {stats.max}.
                </caption>
                <thead>
                    <tr>
