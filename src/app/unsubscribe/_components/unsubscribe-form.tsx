...
export function UnsubscribeForm({
    ...
    async function onSubmit(e: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    body?.error ?? "Unsubscribe failed. Try again.",
                );
            }

            const didFullyUnsubscribe =
                body?.fullUnsubscribe ?? fullUnsubscribe;
                ...
                <p className="text-ink">
                    {resultFullUnsubscribe ? (
                        <>
                            ...
                            has been removed from the list. No more dispatches
                            will arrive.
                        </>
                    ) : (
                        <>
                        ...
                        >
                            <div className="flex flex-col gap-3">
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        setFeedbackReason(
                                            e.target.value as FeedbackReason,
                                        )
                                    }
                                    disabled={feedbackStatus === "submitting"}
                                    aria-invalid={
                                        feedbackStatus === "error" || undefined
                                    }
                                    aria-describedby={
    ...
    return (
        <form onSubmit={onSubmit} className="flex flex-col gap-6" noValidate>
            ...
            <p className="max-w-[60ch] font-sans text-base leading-[1.55] text-ink-soft">
                Tick choices to keep that stream of alerts. Clearing any group
                entirely unsubscribes you from every dispatch.
            </p>
            ...
            {emailLocked ? null : (
                <p className="max-w-[60ch] font-sans text-base leading-[1.55] text-ink-soft">
                    Enter the email address you subscribed with. You only need
                    this manual step if you arrived here without clicking the
                    unsubscribe link in a recent dispatch.
                </p>
            )}
        </form>
    );
