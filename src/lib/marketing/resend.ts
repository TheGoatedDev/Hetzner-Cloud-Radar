...
async function ensureContact(email: string) {
    ...
    if (updated.error?.statusCode !== 404) {
        throw new Error(
            updated.error?.message ?? "Resend contact update failed",
        );
    }
    ...
    if (!created.data) {
        throw new Error(
            created.error?.message ?? "Resend contact create failed",
        );
    }
...
export async function syncMarketingContact(input: {
    ...
    if (!topicUpdate.data && topicUpdate.error) {
        throw new Error(
            topicUpdate.error.message ?? "Resend topic sync failed",
        );
    }
}
