...
export const SERVER_FAMILY_META: Record<string, ServerFamilyMeta> = {
    ...
    cax: {
        ...
        label: "CAX",
        kicker: "ARM Ampere",
        blurb: "Ampere Altra ARM. Originally EU-only; rollout to North America is in progress.",
        fallbackTypes: [
    ...
    ccx: {
        ...
        blurb: "AMD EPYC, dedicated vCPU. The historically-tight line; supply is the story.",
        fallbackTypes: [
    ...
    for (const [family, meta] of Object.entries(SERVER_FAMILY_META)) {
        ...
        if (meta.visible) {
            if (!meta.label || !meta.kicker || !meta.blurb) {
                throw new Error(
                    `${family} visible family metadata is incomplete`,
                );
            }
            if (!meta.fallbackTypes || meta.fallbackTypes.length === 0) {
                throw new Error(
                    `${family} visible family needs fallback types`,
                );
            }
        }
    }
